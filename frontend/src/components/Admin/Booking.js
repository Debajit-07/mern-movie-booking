...
const Booking = () => {
  ...
  const deleteBooking = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/Projectschema/delete/${id}`);
            ...
            {data.length > 0 ? (
              data.map((item) => (
                <tr key={item._id}>
                  ...
                  <td>
                    {item.seatsBooked && item.seatsBooked.length > 0
                      ? item.seatsBooked.join(", ") // Convert array to comma-separated string
                      : "No seats booked"}
                  </td>
